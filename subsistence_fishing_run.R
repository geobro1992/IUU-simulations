...
for(t in 1:(n.years-1)){
  ...
  for(s in 1:n.sites){
    ...
    ps = c(psi.tmp-F.tmp, F.tmp*r, F.tmp*(1-r), (1-psi.tmp))
    ps = ps/sum(ps)
...
#call JAGS from R
(out= jags(data = jags.data, inits = inits, parameters.to.save = parameters, model.file = "subsistence_baseline.txt", 
           n.iter=ni, n.burnin = nb, n.chains= nc,
           n.thin= nt, n.adapt = na, parallel = TRUE))

traceplot(out)
save(out, file = "subsistence_baseline.RData")
load("subsistence_baseline.RData")
...
png("subsistence_baseline.png", width = 6, height = 6, units = "in", res = 600)
...
#call JAGS from R
(out2 = jags(data = jags.data, inits = inits, parameters.to.save = parameters, model.file = "subsistence_with_unreported.txt", 
             n.iter=ni, n.burnin = nb, n.chains= nc,
             n.thin= nt, n.adapt = na, parallel = TRUE))

traceplot(out2)
save(out2, file = "subsistence_with_unreported.RData")
load("subsistence_with_unreported.RData")
...
png("subsistence_with_unreported.png", width = 6, height = 6, units = "in", res = 600)
...
png("subsistence_reporting_predictions.png", width = 7, height = 5, units = "in", res = 600)
...
png("subsistence_comparison.png", width = 12, height = 8, units = "in", res = 600)
...
hist(c(rep(0.99, 40),rep(1, 40)), breaks = 1, col=rgb(1,0,0,0.5), 
     main = "Reporting Rate \n assuming 100% reporting", xlim = c(0.7, 1), ylim = c(0,80), xlab = "", ylab = "")

hist(rest2, breaks = 20, col=rgb(0,0,1,0.5), 
     main = "Reporting Rate \n assuming some unreporting", xlim = c(0.7, 1), xlab = "", ylab = "", add = T)

abline(v = r, col = "grey", lwd = 2)
...
hist(fest2, breaks = 10, col=rgb(0,0,1,0.5), 
     main = "Total Fishing \n assuming some unreporting", xlim = c(0.4, 0.6), xlab = "", ylab = "", add = T)

abline(v = F, col = "grey", lwd = 2)
...
#Bundle data
jags.data <- list(y = y, r = r,                                        # counts of adults and juveniles
                  C=C, gamma = gamma, lambda = lambda, psi = psi,                                        # catch data
                  n.years=ncol(y), n.sites = nrow(y), nreps = nreps) 
...
#call JAGS from R
(out3= jags(data = jags.data, inits = inits, parameters.to.save = parameters, model.file = "subsistence_all_unreported.txt", 
           n.iter=ni, n.burnin = nb, n.chains= nc,
           n.thin= nt, n.adapt = na, parallel = TRUE))

traceplot(out3)
save(out3, file = "subsistence_all_unreported.RData")
...
sms.mu = out$q50$F - F.true
sms.upr = out$q2.5$F - F.true
sms.lwr = out$q97.5$F - F.true
...
ss.mu = out2$q50$F - F.true
ss.upr = out2$q2.5$F - F.true
ss.lwr = out2$q97.5$F - F.true
