...
p = 0.9                # detection
aT = 0.05              # total fishing ~ N slope
alpha = 0.01           # reporting ~ N slope
...
#call JAGS from R
(out= jags(data = jags.data, inits = inits, parameters.to.save = parameters, model.file = "recreational_baseline.txt", 
           n.iter=ni, n.burnin = nb, n.chains= nc,
           n.thin= nt, n.adapt = na, parallel = TRUE))

traceplot(out)
save(out, file = "recreational_baseline.RData")
load("recreational_baseline.RData")
...
#parameters monitored
parameters <- c("Fmax", "alpha", "p", "N", "r")

#MCMC settings
na <- 10000 ; ni <- 30000 ; nt <- 10 ; nb <- 20000 ; nc <- 3

#call JAGS from R
(out2 = jags(data = jags.data, inits = inits, parameters.to.save = parameters, model.file = "recreational_with_unreported.txt", 
             n.iter=ni, n.burnin = nb, n.chains= nc,
             n.thin= nt, n.adapt = na, parallel = TRUE))

traceplot(out2)
save(out2, file = "recreational_with_unreported.RData")
load("recreational_with_unreported.RData")

# posteriors
#lest = out2$sims.list$lambda # true value 20
pest = out2$sims.list$p # true value 0.9
fest = out2$sims.list$Fmax # true value 0.5
...
png("recreational_reporting_predictions.png", width = 7, height = 5, units = "in", res = 600)
...
plot(x, rep(-10, length(x)), type = "p", ylab = "", xlab = " ", cex = 1.5, 
     ylim = c(0.3, 1), xlim = c(0, 100), lwd = 2, pch = 5, axes = FALSE, main = " ")
...
# posteriors
fest2 = out2$mean$Fmax/(1+exp(aT*(-N))) # true value 0.5
rest2 = out2$sims.list$r # true value 0.8

png("recreational_comparison.png", width = 12, height = 8, units = "in", res = 600)
...
hist(FT[,-10], breaks = 10, col=rgb(1,1,1,0.5), 
...
#call JAGS from R
(out.rec.sub = jags(data = jags.data, inits = inits, parameters.to.save = parameters, model.file = "subsistence_with_unreported.txt", 
                    n.iter=ni, n.burnin = nb, n.chains= nc,
                    n.thin= nt, n.adapt = na, parallel = TRUE))

save(out.rec.sub, file = "recreational_mis_subsistence.RData")

load("recreational_mis_subsistence.RData")

F.true = mean(FT[,-10])

rms.mu = out.rec.sub$q50$F - F.true
rms.upr = out.rec.sub$q2.5$F - F.true
rms.lwr = out.rec.sub$q97.5$F - F.true
