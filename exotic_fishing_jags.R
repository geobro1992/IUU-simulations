#########################################################
# recreational fishing (model assumes no unreported harvest)

cat(file="exotic_baseline.txt", " 
    model {
    #priors
...
########################################################################################
# recreational fishing (model assumes some unreported harvest that changes with abundance)

cat(file="exotic_with_unreported.txt", " 
    model {
    #priors
...
# fishing pressure changes with abundance
    ...
    F[s,t] = Fmax/(1+exp(-aT*(-N[s,t])))
    FU[s,t] = Fmax/(1+exp(-alpha*(-N[s,t])))
    r[s,t] = 1-(FU[s,t]/F[s,t])
    
    phi[s,t,1] <- psi-F[s,t]
    phi[s,t,2] <- F[s,t]*r[s,t]
    phi[s,t,3] <- F[s,t]*(1-r[s,t])
    phi[s,t,4] <- 1-psi
